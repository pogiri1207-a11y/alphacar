...
function CarSelector({ title, onSelectComplete, onReset, resetSignal }: CarSelectorProps) {
  ...
  // ✅ [수정된 부분] 기본 트림 선택 시 처리 로직
  const handleBaseTrimChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newBaseTrimId = e.target.value; // 선택된 option의 value (주로 _id)
    ...
    // 1. 선택된 기본 트림 객체를 찾아서 실제 이름(base_trim_name)을 추출
    const selectedBaseTrim = baseTrims.find(t => (t._id || t.name) === newBaseTrimId);
    // 데이터 구조에 따라 base_trim_name 또는 name 필드 사용
    const baseTrimName = selectedBaseTrim?.base_trim_name || selectedBaseTrim?.name || "";

    if (!baseTrimName) {
        console.error("기본 트림 이름을 찾을 수 없습니다.");
        return;
    }

    // 2. API 호출 URL 수정: modelId와 base_trim_name을 모두 전달
    // (이전에는 modelId 파라미터에 newBaseTrimId를 넣어서 오류 발생)
    fetch(`${API_BASE}/vehicles/trims?modelId=${modelId}&base_trim_name=${encodeURIComponent(baseTrimName)}`)
      .then(handleApiResponse)
      .then((data) => {
        if (Array.isArray(data)) {
          setTrims(data);
